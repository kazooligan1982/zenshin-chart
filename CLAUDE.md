...
作業が「完了」とは、以下の全てが満たされた状態を指す:
1. コード修正・ファイル作成・設定変更などの実作業が終わっている
2. コミットメッセージに ClickUp タスクID（#86exXXXXX）が含まれている
3. ClickUp の該当タスクのステータスとコメントが最新の状態に更新されている

**ClickUp に反映されていない作業は、完了していないのと同じ。**
記録は「後でやる別タスク」ではなく、作業の一部である。

---

## レート制限の設計思想（ai_usage_log）

- 現在の方針: fail-open + console.error
- 根拠: ベータ段階ではユーザー体験優先。Anthropic Console の
  ハードリミットが最終防衛線として存在する
